...
This repository contains `proxysql-nagios`, a script for monitoring ProxySQL using Nagios (or a Nagios drop in replacement such as Icinga or Shinken).

The script has the ability to monitor the following:
- Global status variable values
- Connection pool usage (%) per hostgroup
...
- Other table persistence (compare disk, main, and runtime)
...
usage: proxysql-nagios [-h] [-u USER] [-p PASSWD] [-H HOST] [-P PORT]
                       [-d DEFAULTS_FILE] -t {conns,hg,rules,status,var,pers}
                       [-m {pct_used,error}] [-n VAR_NAME] [-r] [-w WARN_THRESH]
                       [-c CRIT_THRESH] [-i INCLUDE_HG] [-g IGNORE_HG]

ProxySQL Nagios Check version 1.2.0
...
Options:
  ...
  -t {conns,hg,rules,status,var,pers}, --type {conns,hg,rules,status,var,pers}
                        ProxySQL check type (one of conns,hg,rules,status,var)
  -m {pct_used,error}, --metric {pct_used,error}
                        metric to report for conns check, or can be used as a 
                        column list when using the pers check
  -n VAR_NAME, --name VAR_NAME
                        ProxySQL variable name to check, or table name when doing pers check
  ...
  -w WARN_THRESH, --warning WARN_THRESH
                        Warning threshold (uses nagios plugin range convention)
  -c CRIT_THRESH, --critical CRIT_THRESH
                        Critical threshold (uses nagios plugin range convention)
